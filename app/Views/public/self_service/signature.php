...
<head>
  ...
  <title>Credencialización | Signature Capture</title>
...
<header class="pt-header">
  <div class="pt-header__inner">
    <div class="pt-header__left">
      <a href="<?= base_url('self-service') ?>" aria-label="Go home">
        <img class="d-logo" src="<?= base_url('assets/img/Instituto_Tecnologico_de_Oaxaca.png') ?>" alt="Logo"
            onerror="this.style.display='none'">
      </a>
      <a href="<?= base_url('self-service') ?>" class="pt-brand pt-brand--link" aria-label="Go home">
      ...
      <div class="pt-firma-layout">
        ...
        <div class="pt-panel pt-panel--main">
          ...
          <div class="pt-stepper">
            <div class="pt-step pt-step--active">1<span>Signature</span></div>
            <div class="pt-step">2<span>Fingerprint</span></div>
            <div class="pt-step">3<span>Photo</span></div>
          </div>

          <h2 class="pt-title">Verify your data and capture your signature</h2>
          <p class="pt-text">
            Check that your information is correct, confirm your data and sign with your <strong>finger</strong> in the box.
          </p>

          <!-- ── Student info ── -->
          <div class="pt-info-card">
            <h3 class="pt-info-card__title">Student Information</h3>
            <dl class="pt-info-grid">
              <div class="pt-info-item">
                <dt>Name</dt>
                <dd><?= esc($student['full_name'] ?? 'N/A') ?></dd>
              </div>
              <div class="pt-info-item">
                <dt>Control / Registration No.</dt>
                <dd><?= esc($student['control_number'] ?? $student['registration_number'] ?? 'N/A') ?></dd>
              </div>
              <div class="pt-info-item">
                <dt>Major</dt>
                <dd><?= esc($student['major_name'] ?? 'N/A') ?></dd>
              </div>
            </dl>
          </div>
          ...
          <label class="pt-confirm" id="confirmLabel">
            ...
            <span class="pt-confirm__text">
              I confirm that my data is <strong>correct</strong> and I accept that my signature will be used on the credential.
            </span>
          </label>
          ...
          <div class="pt-firma-wrap pt-firma-wrap--locked" id="firmaWrap">
            ...
            <div class="pt-firma-placeholder" id="firmaPlaceholder">
              ...
              <span id="placeholderText">Confirm your data first to sign</span>
            </div>
            <div class="pt-firma-lock-overlay" id="firmaLock">
              ...
              <span>Confirm your data above to unlock</span>
            </div>
          </div>

          <!-- Actions -->
          <div class="pt-firma-actions">
            <button type="button" id="btnClear" class="pt-btn pt-btn--secondary pt-btn--auto" disabled>
              Clear signature
            </button>
          </div>

          <div id="signatureStatus" class="pt-firma-status">
            Confirm your data and draw your signature to continue.
          </div>

          <!-- Form to submit -->
          <form id="signatureForm" method="post" action="<?= base_url('self-service/signature') ?>">
            <?= csrf_field() ?>
            <input type="hidden" name="signature" id="signaturePngInput" value="">

            <div class="pt-firma-submit-group">
              <button type="submit" id="btnSave" class="pt-btn pt-btn--success" disabled>
          ...
          <div class="pt-note" style="margin-top:14px;">
            Your signature will be saved securely and used only for the credential.
          </div>
        </div>
...
<footer class="d-footer">
  ...
  <span>Developed with <img class="d-ico-img" src="<?= base_url('assets/img/heartSVG.svg') ?>" alt=""></span>
</footer>
...
(() => {
  ...
  const hiddenInput = document.getElementById('signaturePngInput');
  const statusEl    = document.getElementById('signatureStatus');
  const form        = document.getElementById('signatureForm');
  const confirmCheck = document.getElementById('confirmCheck');
  const confirmLabel = document.getElementById('confirmLabel');

  if (!canvas || !wrap || !btnClear || !btnSave || !hiddenInput || !form || !confirmCheck) return;
  ...
  function resetPad() {
    ...
    hiddenInput.value = '';
    if (unlocked) {
      statusEl.textContent = 'Draw your signature to continue.';
    }
  }
  ...
  confirmCheck.addEventListener('change', () => {
    ...
    if (checked) {
      ...
      placeholderText.textContent = 'Touch here and draw your signature';
      statusEl.textContent = 'Data confirmed ✓ — Draw your signature to continue.';
    } else {
      unlocked = false;
      ...
      statusEl.textContent = 'Confirm your data first to sign.';
    }
  });
  ...
  function move(e) {
    ...
    if (!hasInk) {
      ...
      statusEl.textContent = 'Signature detected. When finished, tap "Save signature and continue".';
    }
    e.preventDefault();
  }
  ...
  form.addEventListener('submit', (e) => {
    if (!unlocked) {
      e.preventDefault();
      statusEl.textContent = 'You must confirm that your data is correct before continuing.';
      return;
    }
    ...
    if (hasInk) {
      ...
      e.preventDefault();
      statusEl.textContent = 'Draw your signature before saving.';
    }
  });

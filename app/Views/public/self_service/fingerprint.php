...
<head>
  ...
  <title>Credentialing | Fingerprint Capture</title>
      ...
      <div class="pt-firma-layout">
        ...
        <div class="pt-panel pt-panel--main">
          ...
          <div class="pt-stepper">
            ...
            <div class="pt-step pt-step--active">2<span>Fingerprint</span></div>
            <div class="pt-step">3<span>Photo</span></div>
          </div>

          <h2 class="pt-title">Fingerprint Capture</h2>
          <p class="pt-text">Confirm your data to unlock the HID fingerprint sensor.</p>

          <div class="pt-info-card">
            <h3 class="pt-info-card__title">Data Summary</h3>
            <dl class="pt-info-grid">
              <div class="pt-info-item"><dt>Name</dt><dd><?= esc($student['full_name'] ?? 'N/A') ?></dd></div>
              <div class="pt-info-item"><dt>Control No.</dt><dd><?= esc($student['control_number'] ?? $student['registration_number'] ?? 'N/A') ?></dd></div>
            </dl>
          </div>
          ...
          <label class="pt-confirm" id="confirmLabel">
            ...
            <span class="pt-confirm__text">I confirm that my data is <strong>correct</strong>.</span>
          </label>
          ...
          <div class="pt-huella-panel pt-huella-panel--locked" id="huellaPanel">
            <div class="pt-lock-overlay" id="lockOverlay">
              ...
              <span>Confirm above to unlock</span>
            </div>
            ...
            <h3 class="pt-modal-title" id="statusTitle">Sensor Locked</h3>
            <p id="huellaStatusText" class="pt-firma-status">Waiting for activation.</p>
            
            <div class="pt-progress-bar"><div class="pt-progress-fill" id="progressFill"></div></div>

            <div style="display:flex; gap:10px; justify-content:center; margin-top:20px;">
              <button type="button" id="btnIniciarCaptura" class="pt-btn pt-btn--secondary" style="display:none; width:100%">Start Fingerprint Scan</button>
            </div>
          </div>

          <form id="huellaForm" method="get" action="<?= base_url('self-service/photo') ?>" style="margin-top:20px;">
            <button type="submit" id="btnFinalizar" class="pt-btn pt-btn--success" disabled style="width:100%">Finish and continue to Photo</button>
          </form>
        </div>
      </div>
    </section>
...
(() => {
  ...
  const studentId    = '<?= esc((string) ($student['id'] ?? 0)) ?>';
  ...
  confirmCheck.addEventListener('change', () => {
    if (confirmCheck.checked) {
      ...
      statusTitle.textContent = 'Sensor Unlocked';
      huellaStatus.textContent = 'Preparing connection...';
      verificarStatus();
    } else {
  ...
  async function verificarStatus() {
    try {
      const res = await fetch('<?= base_url('huella/tiene-huella') ?>', {
        method: 'POST', body: new URLSearchParams({ alumno_id: studentId })
      });
      const data = await res.json();
      btnCapturar.style.display = 'block';
      btnCapturar.textContent = data.tiene_huella ? '👆 Verify Identity' : '📱 Register New Fingerprint';
      huellaStatus.textContent = 'Ready. Press the button to use the HID reader.';
    } catch (e) {
      huellaStatus.textContent = 'Error connecting to the server.';
    }
  }
  ...
  btnCapturar.onclick = async () => {
    ...
    statusTitle.textContent = 'Capturing...';
    huellaStatus.textContent = 'Place your finger on the HID reader. Lift and place it several times if necessary.';
    ...
    try {
      const isRegister = btnCapturar.textContent.includes('Register');
      
      if (isRegister) {
        // REGISTRATION
        const opt = await fetch('<?= base_url('huella/registro-challenge') ?>', {
          method: 'POST', body: new URLSearchParams({ alumno_id: studentId, nombre: '<?= esc($student['full_name'] ?? '') ?>' })
        }).then(r => r.json());
        ...
        huellaStatus.textContent = 'Verifying with server...';
        ...
        if (!ver.success) throw new Error(ver.error || 'Error saving');
      } else {
        // VERIFICATION
        ...
        if (!ver.verified) throw new Error('Could not verify fingerprint.');
      }
      ...
      statusTitle.textContent = 'Capture Successful!';
      huellaStatus.textContent = 'Your fingerprint has been processed correctly.';
      btnCapturar.style.display = 'none';
      ...
      huellaStatus.textContent = err.message || 'The process was cancelled or failed.';
    }
  };
})();
</script>

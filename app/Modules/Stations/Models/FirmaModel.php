...
/**
 * Model for handling signature captures and their associated queue.
 */
class FirmaModel extends Model
{
    ...
    {
        return $this->queueService->getQueue('signature', $limit);
    }

    public function getCurrentByTurno(int $ticketId): ?array
    {
        return $this->queueService->getCurrent('signature', $ticketId);
    }

    public function getNextPending(): ?array
    {
        return $this->queueService->getCurrent('signature');
    }

    public function getByAlumnoId(int $studentId): ?array
    {
        return $this->queueService->getByStudentId('signature', $studentId);
    }

    /**
     * Saves a signature from a data URL, creating a file record and updating student/ticket.
     */
    public function saveFirma(int $studentId, int $ticketId, string $signatureDataUrl): array
    {
        $current = $this->getCurrentByTurno($ticketId);

        if (!$current || $current['student_id'] != $studentId) {
            throw new RuntimeException('El turno seleccionado ya no está disponible para captura de firma.');
        }

        [$binary, $mime, $extension] = $this->decodeImageDataUrl($signatureDataUrl);

        $relativePath = 'uploads/firmas/signature_' . $studentId . '_' . date('Ymd_His') . '_' . bin2hex(random_bytes(4)) . '.' . $extension;
        $absolutePath = FCPATH . $relativePath;
        $dir = dirname($absolutePath);

        if (!is_dir($dir) && !mkdir($dir, 0775, true) && !is_dir($dir)) {
            throw new RuntimeException('No se pudo preparar el directorio para guardar la firma.');
        }

        if (file_put_contents($absolutePath, $binary) === false) {
            throw new RuntimeException('No se pudo escribir el archivo de la firma.');
        }
        ...
        if (!$this->db->transStatus()) {
            @unlink($absolutePath);
            throw new RuntimeException('No fue posible guardar la firma en la base de datos.');
        }
    ...
    private function decodeImageDataUrl(string $dataUrl): array
    {
        ...
        if ($binary === false) {
            throw new RuntimeException('No se pudo decodificar la firma enviada.');
        }

        $extension = match ($mime) {
            'image/png'  => 'png',
            'image/jpeg' => 'jpg',
            default      => throw new RuntimeException('El tipo de imagen de la firma no está soportado.'),
        };

...
class FirmaModel extends Model
{
    ...
    public function saveFirma(int $alumnoId, int $turnoId, string $firmaPng): array
    {
        ...
        [$binary, $mime, $extension] = $this->decodeImageDataUrl($firmaPng);

        $relativePath = 'uploads/firmas/firma_' . $alumnoId . '_' . date('Ymd_His') . '_' . bin2hex(random_bytes(4)) . '.' . $extension;
    ...
    private function baseQueueBuilder()
    {
        ...
        return $this->db->table('turnos t')
            ...
            ->join('turno_eventos te_firma', "te_firma.turno_id = t.id_turno AND te_firma.tipo_evento = 'firma_guardada'", 'left')
            ->where('t.es_activo', 1)
            ->where('t.fecha_expira >=', $ahora)
            ->where('te_firma.turno_id IS NULL', null, false)
            ->groupStart()
